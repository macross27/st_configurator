...
export class SceneManager {
    ...
    /**
     * Create a visible plane geometry for texture display when hybrid system is disabled
     */
    createMinimalFallbackModel() {
        console.log('🔧 Creating visible plane model for texture display');
    ...
    setModelSetVisibility(setOption, designType = 'regulan', neckType = 'std_a') {
        ...
        // Keep fallback model visible when not using hybrid system
        if (this.model && !this.useHybridSystem) {
            console.log('🔧 Keeping fallback model visible for texture display');
            this.model.visible = true;

            // Ensure all meshes in fallback model are visible
            this.model.traverse((child) => {
                if (child.isMesh) {
                    child.visible = true;
                    console.log(`🔧 Showing fallback mesh for texture display: ${child.name || 'unnamed'}`);
                }
            });
        } else if (this.model && this.useHybridSystem) {
            console.log('🔧 Hiding fallback model (hybrid system active)');
