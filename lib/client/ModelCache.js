...
export class ModelCache {
            ...
            this.gltfLoader.load(
                ...
                async (gltf) => {
                    try {
                        ...
                        // Dispose original materials and apply SHARED material instance
                        const sharedMaterial = this.getModelMaterial(modelPath);
                        let meshCount = 0;
                        ...
                        modelObject.traverse((child) => {
                            if (child.isMesh) {
                                ...
                                // CRITICAL: Assign the SAME shared material instance to ALL meshes
                                child.material = sharedMaterial;

                                // Verify material assignment
                                console.log(`🔍 Mesh "${child.name}" assigned material ID: ${child.material.id} (should match master: ${this.masterMaterial?.id})`);
                            }
                        });

                        console.log(`🎨 Applied SHARED material (ID: ${sharedMaterial.id}) to ${meshCount} meshes in ${modelPath}`);
    ...
    /**
     * Create shared material instance for a model (same material reference)
     */
    createModelMaterial(modelPath) {
        ...
        // CRITICAL FIX: ALL models must reference the exact same material instance
        // Do not create new material instances - share the master material completely
        this.modelMaterials.set(modelPath, this.masterMaterial);

        console.log(`🎨 Assigned SHARED material to ${modelPath}. Master material ID:`, this.masterMaterial.id);
        console.log(`🎨 Material references count: ${this.modelMaterials.size} models using same material`);
        return this.masterMaterial;
    }
    ...
    /**
     * Apply shared texture to a specific model with shared material
     */
    applySharedTexture(modelObject, modelPath) {
        ...
        // Get the shared material for this model (same reference for all)
        const modelMaterial = this.getModelMaterial(modelPath);

        // Update the shared material with new texture
        if (modelMaterial) {
            modelMaterial.map = this.sharedTexture;
            modelMaterial.needsUpdate = true;

            // Ensure all meshes in this model use the shared material
            let meshCount = 0;
            modelObject.traverse((child) => {
                if (child.isMesh) {
                    meshCount++;
            ...
            console.log(`🎨 Applied shared material (ID: ${modelMaterial.id}) to ${meshCount} meshes in ${modelPath}`);
        }
    }
    
    /**
     * Update shared texture for all cached models (single material reference)
     */
    updateSharedTextureForAll() {
        ...
        // Update master material (all models share this single material)
        if (this.masterMaterial) {
            this.masterMaterial.map = this.sharedTexture;
            this.masterMaterial.needsUpdate = true;
        }

        console.log(`🎨 Updated shared texture for ${this.loadedModels.size} cached models (single shared material)`);
    }
    
    /**
     * Apply texture to a specific model (uses shared material)
     */
    applyTextureToModel(modelObject, texture, modelPath) {
        // Update the shared master material (affects all models)
        if (this.masterMaterial) {
            this.masterMaterial.map = texture;
            this.masterMaterial.needsUpdate = true;

            // Ensure all meshes use the shared material
            modelObject.traverse((child) => {
                if (child.isMesh) {
                    child.material = this.masterMaterial;
                }
            });
        }
    }
    ...
    /**
     * Count total materials in all cached models (should be exactly 1 for shared system!)
     */
    countTotalMaterials() {
                ...
                if (child.isMesh && child.material) {
                    if (Array.isArray(child.material)) {
                        child.material.forEach(mat => {
                            ...
                            if (!materialBreakdown.has(mat.id)) {
                                materialBreakdown.set(mat.id, []);
                            }
                            materialBreakdown.get(mat.id).push(`${modelPath}:${child.name}`);
                        });
                    } else {
                        uniqueMaterials.add(child.material.id);
                        if (!materialBreakdown.has(child.material.id)) {
                            materialBreakdown.set(child.material.id, []);
                        }
                        materialBreakdown.get(child.material.id).push(`${modelPath}:${child.name}`);
                    }
                }
            });
        }
        ...
        console.log(`🔍 Total unique materials: ${uniqueMaterials.size} (SHOULD BE 1 for shared system!)`);
        console.log(`🔍 Master material ID: ${this.masterMaterial?.id || 'none'}`);
        console.log(`🔍 Material breakdown:`);

        for (const [materialId, meshes] of materialBreakdown.entries()) {
            const isShared = materialId === this.masterMaterial?.id;
            console.log(`🔍   Material ${materialId} ${isShared ? '✅ SHARED' : '❌ SEPARATE'}: ${meshes.length} meshes`);
            console.log(`🔍     Meshes: ${meshes.join(', ')}`);
        }

        if (uniqueMaterials.size > 1) {
            console.error(`❌ HYBRID GLB SYSTEM BROKEN: ${uniqueMaterials.size} different materials found, should be 1!`);
        } else {
            console.log(`✅ HYBRID GLB SYSTEM WORKING: All models use single shared material`);
        }
    ...
    dispose() {
        ...
        // Clear model material references (all reference the same master material)
