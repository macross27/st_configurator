...
export class LayerManager {
    ...
    async updateBaseTexture(primaryColor = '#ff6600', secondaryColor = '#0066ff') {
            ...
            if (window.uniformConfigurator && window.uniformConfigurator.patternCompositor) {
                try {
                    ...
                    if (currentPattern) {
                        ...
                        const patternTexture = await window.uniformConfigurator.patternCompositor.createCompositeTexture(currentPattern, colors);

                        // Draw pattern texture to canvas
