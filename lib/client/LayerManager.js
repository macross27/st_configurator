...
export class LayerManager {
    constructor() {
        ...
        this.textureContributors = new Set();
        this.contributorValues = new Map();
        this.contributorChangeListeners = new Map();
        this.setupTextureUpdateSystem();
    }
                ...
                if (window.uniformConfigurator && window.uniformConfigurator.patternCompositor) {
                    try {
                        ...
                        if (currentPattern && currentPattern.name) {
                            console.log(`🎨 LayerManager using pattern texture for base: ${currentPattern.name}`);

                            // Validate pattern has required data
                            if (!currentPattern.files || currentPattern.files.length === 0) {
                                console.warn(`⚠️ Pattern ${currentPattern.name} missing file data, using solid color`);
                                this.textureContext.clearRect(0, 0, width, height);
                                this.textureContext.fillStyle = this.currentPrimaryColor;
                                this.textureContext.fillRect(0, 0, width, height);
                                return;
                            ...
                            await new Promise((resolve, reject) => {
                                patternImg.onload = () => {
                                    ...
                                    console.log(`✅ Pattern texture loaded and applied to base texture: ${currentPattern.name}`);
                                    resolve();
                                };
                                patternImg.onerror = (error) => {
                                    console.error(`❌ Failed to load pattern image for ${currentPattern.name}:`, error);
                            ...
                            console.log('🎨 LayerManager no current pattern, using solid color');
                            this.textureContext.clearRect(0, 0, width, height);
                            this.textureContext.fillStyle = this.currentPrimaryColor;
                            this.textureContext.fillRect(0, 0, width, height);
                        }
    ...
    dispose() {
        ...
        // Clean up contributor monitoring
        this.disposeContributorMonitoring();
    ...
    /**
     * Setup centralized texture update system that monitors all texture contributors
     */
    setupTextureUpdateSystem() {
        console.log('🎯 Setting up centralized texture update system');

        // Define all texture contributors and their selectors/sources
        this.textureContributorConfig = {
            // Pattern colors
            'pattern-color-1': { type: 'colorPicker', selector: '#pattern-color-1' },
            'pattern-color-2': { type: 'colorPicker', selector: '#pattern-color-2' },
            'pattern-color-3': { type: 'colorPicker', selector: '#pattern-color-3' },
            'pattern-color-4': { type: 'colorPicker', selector: '#pattern-color-4' },
            'pattern-color-5': { type: 'colorPicker', selector: '#pattern-color-5' },

            // Additional texture colors
            'pants-color-1': { type: 'colorPicker', selector: '#pants-color-1' },
            'pants-color-2': { type: 'colorPicker', selector: '#pants-color-2' },
            'neck-color-1': { type: 'colorPicker', selector: '#neck-color-1' },
            'neck-color-2': { type: 'colorPicker', selector: '#neck-color-2' },

            // Pattern selection
            'current-pattern': { type: 'pattern', source: 'patternManager' },

            // Number textures (if applicable)
            'number-textures': { type: 'numberTextures', source: 'custom' }
        };

        this.initializeContributorMonitoring();
    }

    /**
     * Initialize monitoring for all texture contributors
     */
    initializeContributorMonitoring() {
        // Setup DOM observers after a brief delay to ensure elements are ready
        setTimeout(() => {
    ...
    /**
     * Setup DOM observers for color pickers and other UI elements
     */
    setupDOMObservers() {
        console.log('🔍 Setting up DOM observers for texture contributors');

        Object.entries(this.textureContributorConfig).forEach(([key, config]) => {
            if (config.type === 'colorPicker') {
                const element = document.querySelector(config.selector);
                if (element) {
                    // Store initial value
                    this.contributorValues.set(key, element.value);

                    // Setup change listener
                    const listener = () => {
                        this.onContributorChange(key, element.value);
                    };

                    element.addEventListener('input', listener);
                    element.addEventListener('change', listener);

                    this.contributorChangeListeners.set(key, { element, listener });

                    console.log(`✅ Monitoring ${key}: ${element.value}`);
                } else {
                    console.warn(`⚠️ Could not find element for ${key}: ${config.selector}`);
                }
            }
        });

        // Setup pattern change monitoring
        this.setupPatternChangeMonitoring();
    }

    /**
     * Setup pattern change monitoring
     */
    setupPatternChangeMonitoring() {
        // Monitor pattern changes through global pattern manager
        if (window.uniformConfigurator && window.uniformConfigurator.patternManager) {
            const originalUpdatePattern = window.uniformConfigurator.patternManager.updatePattern;
            const layerManager = this;

            window.uniformConfigurator.patternManager.updatePattern = function(patternData) {
                const result = originalUpdatePattern.call(this, patternData);
                layerManager.onContributorChange('current-pattern', patternData?.name || 'none');
                return result;
            };

            console.log('✅ Pattern change monitoring setup complete');
        }
    }

    /**
     * Capture initial values for all contributors
     */
    captureInitialValues() {
        console.log('📸 Capturing initial contributor values');

        // Capture current pattern
        if (window.uniformConfigurator && window.uniformConfigurator.patternManager) {
            const currentPattern = window.uniformConfigurator.patternManager.getCurrentPattern();
            this.contributorValues.set('current-pattern', currentPattern?.name || 'none');
        }

        console.log('📸 Initial contributor values:', Object.fromEntries(this.contributorValues));
    }

    /**
     * Handle contributor value changes
     */
    onContributorChange(contributorKey, newValue) {
        const oldValue = this.contributorValues.get(contributorKey);

        if (oldValue === newValue) {
            return; // No change
        }

        console.log(`🎨 Texture contributor changed: ${contributorKey} ${oldValue} → ${newValue}`);

        // Update stored value
        this.contributorValues.set(contributorKey, newValue);

        // Trigger systematic texture update
        this.triggerSystematicTextureUpdate(contributorKey, oldValue, newValue);
    }

    /**
     * Trigger systematic texture update when any contributor changes
     */
    triggerSystematicTextureUpdate(changedContributor, oldValue, newValue) {
        console.log(`🔄 Triggering systematic texture update due to: ${changedContributor}`);

        // Clear any existing caches that might be affected
        this.clearRelevantCaches(changedContributor);

        // Force a complete texture rebuild
        this.markBaseDirty();
        this.updateTexture(true); // Force update

        console.log('✅ Systematic texture update completed');
    }

    /**
     * Clear relevant caches when contributors change
     */
    clearRelevantCaches(changedContributor) {
        // Clear pattern compositor caches if pattern-related changes
        if (changedContributor.includes('pattern') || changedContributor.includes('color')) {
            if (window.uniformConfigurator && window.uniformConfigurator.patternCompositor) {
                window.uniformConfigurator.patternCompositor.clearCache();
                window.uniformConfigurator.patternCompositor.clearTextureCache();
                console.log('🧹 Cleared pattern compositor caches');
            }
        }

        // Clear LayerManager caches
        this.textMetricsCache.clear();
        this.dirtyRegions.add('all');
    }

    /**
     * Manually trigger texture update (for external calls)
     */
    forceTextureUpdate(reason = 'external') {
        console.log(`🔄 Force texture update requested: ${reason}`);
        this.triggerSystematicTextureUpdate('manual', null, null);
    }

    /**
     * Register external texture contributor (for dynamic contributors)
     */
    registerTextureContributor(key, getValue, setupMonitoring) {
        console.log(`📝 Registering external texture contributor: ${key}`);

        this.textureContributorConfig[key] = {
            type: 'external',
            getValue: getValue,
            setupMonitoring: setupMonitoring
        };

        // Setup monitoring if provided
        if (setupMonitoring) {
            setupMonitoring((newValue) => {
                this.onContributorChange(key, newValue);
            });
        }

        // Capture initial value
        if (getValue) {
            this.contributorValues.set(key, getValue());
        }
    }
     ...
     * Clean up contributor monitoring
     */
    disposeContributorMonitoring() {
        // Remove all DOM event listeners
        this.contributorChangeListeners.forEach(({ element, listener }, key) => {
            if (element) {
        ...
        // Clear maps
        this.contributorChangeListeners.clear();
        this.contributorValues.clear();
        this.textureContributors.clear();

        console.log('🧹 Contributor monitoring disposed');
    }
}