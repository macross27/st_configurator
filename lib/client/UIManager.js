...
export class UIManager {
    ...
    createLayerElement(layer, selectedLayer) {
        ...
        const imageErrorIndicator = layer.hasImageError ? 
            `<span class="image-error-indicator" title="${layer.imageErrorReason || 'Image missing'}">⚠️</span>` : '';
        
        div.innerHTML = `
            <div class="layer-header">
                <span class="layer-name">${layer.name}${imageErrorIndicator}</span>
                <div class="layer-controls">
                    <button class="layer-control-btn lock" title="${layer.locked ? 'Unlock layer' : 'Lock layer'}">${layer.locked ? '🔒' : '🔓'}</button>
                    <button class="layer-control-btn delete-x" title="Delete layer">×</button>
                </div>
            ...
            if (urlRegex.test(part)) {
                ...
                urlSpan.onclick = async (e) => {
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            urlSpan.textContent = originalText;
                        }, 1500);
                    } catch (err) {
    ...
    showSubmissionDialog() {
        ...
        dialog.innerHTML = `
            <div class="submission-dialog-content">
                ...
                <h3 class="submission-title">작업물 제출 중...</h3>
                <p class="submission-message">잠시만 기다려주세요. 작업물을 처리하고 있습니다.</p>
                <div class="submission-progress">
                    <div class="submission-progress-bar">
                        <div class="submission-progress-fill"></div>
                    </div>
                    <span class="submission-progress-text">처리중...</span>
                </div>
            </div>
        `;
    ...
    showSubmissionSuccess(message, shareableUrl) {
        ...
        content.innerHTML = `
            ...
            <h3 class="submission-title success">제출 완료!</h3>
            <div class="submission-message">
                <p>작업물이 성공적으로 제출되었습니다!</p>
                <div class="url-container">
                    <label>공유 URL:</label>
                    <div class="clickable-url" data-url="${shareableUrl}" title="클릭하여 클립보드에 복사">
                        ${shareableUrl}
                    </div>
                    <small class="url-instruction">📋 URL을 클릭하면 클립보드에 복사됩니다.</small>
                </div>
            </div>
            <button class="submission-close-btn" title="창 닫기">창닫기</button>
        `;
        
        dialog.classList.add('success');
        
        // Add close button handler
        const closeBtn = content.querySelector('.submission-close-btn');
        if (closeBtn) {
        ...
        if (urlElement) {
            urlElement.addEventListener('click', async () => {
                ...
                try {
                    ...
                    urlElement.textContent = '✅ 클립보드에 복사됨!';
                    ...
                    this.showNotification('클립보드 복사에 실패했습니다', 'error', 5000);
                }
            });
        }
    }
    ...
    showSubmissionError(message) {
        ...
        content.innerHTML = `
            <div class="submission-error-icon">❌</div>
            <h3 class="submission-title error">제출 실패</h3>
            <p class="submission-message">${message}</p>
            <button class="submission-retry-btn" onclick="document.querySelector('.submission-dialog-overlay').parentNode.removeChild(document.querySelector('.submission-dialog-overlay'))">확인</button>
        `;
