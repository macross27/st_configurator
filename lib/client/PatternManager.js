export class PatternManager {
    ...
    selectPattern(patternName) {
        ...
        // Validate pattern has all required data
        if (!patternData.name || !patternData.files || patternData.files.length === 0) {
            console.error(`❌ Pattern ${patternName} missing required data:`, patternData);
            return;
        }

        // Set current pattern
        this.currentPattern = patternData;
        console.log(`✅ Pattern data set for: ${this.currentPattern.name}`, {
            layerCount: this.currentPattern.layerCount,
            filesCount: this.currentPattern.files.length,
            texturePath: this.currentPattern.texturePath
        });

        // Update color pickers
        this.updateColorPickers();

        // Trigger pattern change callback with slight delay to ensure UI updates are complete
        setTimeout(() => {
            if (this.onPatternChange) {
                console.log(`🔄 Triggering pattern change callback for: ${this.currentPattern.name}`);
    ...
    manualInitializeColorListeners() {
        ...
        // Note: Body color pickers have been removed - using pattern colors and keeping neck/pants
        const colorPickers = document.querySelectorAll('#pattern-color-1, #pattern-color-2, #pattern-color-3');
        console.log(`Found ${colorPickers.length} pattern color pickers`);
        ...
        colorPickers.forEach((picker, index) => {
            ...
            console.log(`✅ Added manual listeners to pattern picker ${layerIndex} (${picker.id})`);
        });
    ...
    setRestorationMode(isRestoring) {
        this.isRestoring = isRestoring;
        console.log(`🔧 PatternManager restoration mode: ${isRestoring ? 'ON' : 'OFF'}`);
    }
    ...
    onDesignTypeChange(newDesignType) {
        ...
        // Clear all pattern compositor caches to prevent stale pattern textures
        if (this.patternCompositor) {
            console.log('🗑️ Clearing PatternCompositor caches due to design type change');
            this.patternCompositor.clearCache();
            this.patternCompositor.clearTextureCache();
        } else if (window.uniformConfigurator?.patternCompositor) {
            console.log('🗑️ Clearing PatternCompositor caches via global reference due to design type change');
            window.uniformConfigurator.patternCompositor.clearCache();
            window.uniformConfigurator.patternCompositor.clearTextureCache();
        }

        // Skip auto-selection during restoration to prevent overriding restored patterns
        if (this.isRestoring) {
            console.log(`⏸️ Skipping auto-pattern selection during restoration`);
            return;
        }
        ...
        // Clear current pattern to force fresh loading
        this.currentPattern = null;

        setTimeout(() => {
            this.selectPattern(defaultPattern);

            // Force LayerManager texture update to ensure new pattern is applied
            if (window.uniformConfigurator?.layerManager) {
                console.log('🔄 Forcing LayerManager texture update after design type change');
                window.uniformConfigurator.layerManager.forceTextureUpdate('design-type-change');
            }
        }, 100);
    }
    ...
    forceAttachEventListeners() {
        console.log('🔧 Force re-attaching event listeners to pattern color pickers...');

        const colorPickers = document.querySelectorAll('#pattern-color-1, #pattern-color-2, #pattern-color-3');
        console.log(`Found ${colorPickers.length} existing pattern color pickers`);
